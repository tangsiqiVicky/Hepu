<template>
  <div class="loginContainer">
    ...
    <!--注册过的用户可凭账号密码登录-->
    <!--</p>-->
    <div class="login_tips">首次注册密码为手机号后六位</div>
    <div class="login_container" @click="mobileLogin">登录</div>
    <router-link to="/registration" class="to_forget" v-if="!loginWay">注册？</router-link>
...
<script>
  ...
  import {mobileCode, sendLogin, accountLogin,getUser,getjsapi_signature,getOpenid} from '../../service/getData'
...
//      })
    ...
    methods: {
      ...
      isWeixin(){
        let ua = window.navigator.userAgent.toLowerCase(); // 判断是不是微信浏览器
      ...
      async isLoginWeixin(user_id){

          const res = await this.request(user_id);
          console.log(res);

      },
      // 获取code
      getCode(scope, redirectUri, state) {
        const accessTokenUrl='https://open.weixin.qq.com/connect/oauth2/authorize?';
        var redirectURL = encodeURIComponent(redirectUri);
        var oauthURL = accessTokenUrl;
        oauthURL += "appid=" + this.appid;
      ...
      async mobileLogin(){
        if (this.loginWay) {
          ...
          //用户名登录
          let res = await accountLogin(this.userAccount, this.passWord );
          if(res.data[0].user_id){
            ...
            if(!res.data[0].openid){
              if(this.isWeixin()){
                await this.isLoginWeixin(res.data[0].user_id);
              }else{
                this.$router.go(-1);
              }
            }else{
...
//          if(res.data[0].openid){
//            let infos = await getUser(res.data[0].user_id);
//            this.userInfo = infos.data[0]
//          }else {
//            await this.isLoginWeixin(res.data[0].user_id);
////              if(this.code){
////                let res = await getOpenid(this.code);
